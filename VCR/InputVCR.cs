...
{
	public Recording.InputInfo[] inputsToRecord; 	// list of axis and button names ( from Input manager) that should be recorded
	...
	public InputVCRMode mode { get { return _mode; } }

	private float nextPosSyncTime = -1f;
	private float realRecordingTime;
	private Recording currentRecording;		// the recording currently in the VCR. Copy or ToString() this to save.

	public float currentTime{ get { return currentFrame / (float)currentFrameRate; } }
	public int currentFrame	{ get; private set; }	// current frame of recording/playback

	public int currentFrameRate{
		get {
			if ( currentRecording == null ) return recordingFrameRate;
			else 							return currentRecording.frameRate;
		}
	}


	private Queue<Recording.FrameProperty>	 nextPropertiesToRecord = new Queue<Recording.FrameProperty>();	// if SyncLocation or SyncProperty are called, this will hold their results until the recordstring is next written to
	private Dictionary<string, Recording.InputInfo> lastFrameInputs = new Dictionary<string, Recording.InputInfo>();	// list of inputs from last frame (for seeing what buttons have changed state)
	private Dictionary<string, Recording.InputInfo> thisFrameInputs = new Dictionary<string, Recording.InputInfo>();
	...
	public void Play()
	{
		...
		if ( mode == InputVCRMode.Pause ) _mode = InputVCRMode.Playback;
		else
		{
	...
	public bool GetMouseButton( int buttonNum )
	{
		if ( _mode == InputVCRMode.Pause )
			return false;
		string mouseButtonName =  "mousebutton" + buttonNum;
